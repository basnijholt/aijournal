...
| **CLI option/argument wiring** | `FBT003` (26), `B008` (16), `B008` / `typer.Argument` (3) | `src/aijournal/cli.py` | Typer discourages calling `Option`/`Argument` in parameter defaults. Wrap each command definition in a helper decorator (e.g., `def option_workspace() -> typer.Option`) or move the definitions into module-level constants. Addressing this will touch most CLI commands; tackle per-command to keep diffs reviewable. |
