...
- **RAG-Enhanced Profile Updates**: Profile updates now use retrieval-augmented generation (RAG) to strengthen claims and consolidate patterns into facets. Pre-retrieval runs 4 hardcoded queries against the vector index to fetch up to 40 relevant historical chunks. Facet consolidation triggers only when 10+ new claims accumulate (tracked in `derived/profile_update_meta.yaml`), and the LLM answers 6 specific questions about current focus, blockers, deep work timing, routines, values/themes, and personality traits. Each facet category has minimum chunk thresholds (2 for planning, 3 for habits, 5 for values/traits) to ensure evidence-based consolidation. Claims can now cite both today's entry (`evidence_entry`) and historical chunks (`evidence_chunk_ids`) for stronger provenance. See `prompts/profile_update.md` Part 2 for the full consolidation workflow.
